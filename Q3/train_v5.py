...
class SACAgent:
    def __init__(self, obs_dim, act_dim, act_limit, batch_size = 64, gamma=0.99, alpha=1, tau=0.005, lr = 1e-4):
        self.actor = Actor(obs_dim, act_dim, act_limit)
        self.critic = Critic(obs_dim, act_dim)
        self.critic_target = Critic(obs_dim, act_dim)

        self.actor.load_state_dict(torch.load("SAC_actor_6.pth"))
        self.critic.load_state_dict(torch.load("SAC_critic_6.pth"))
        ...
        self.log_alpha = torch.tensor(np.log(0.00001)).to(device)
