...
class Critic(nn.Module):
    ...
    def forward(self, obs, act):
        ...
        return self.q1(xu), self.q2(xu), self.v(obs)

class SACAgent:
    def __init__(self, obs_dim, act_dim, act_limit, batch_size = 256, gamma=0.99, alpha=0.2, tau=0.005, lr = 3e-4):
    ...
    def update(self):
        ...
        q1_pi, q2_pi, v = self.critic(state, act_new)
        q_pi = torch.min(q1_pi, q2_pi)
        target_v = q_pi.detach() - self.alpha * logp.detach()

        # Target action
        with torch.no_grad():
            next_act, _ = self.actor.sample(next_state)
            _, _, next_v = self.critic_target(next_state, next_act)
            target_q = (rew + self.gamma * (1 - done) * next_v.squeeze(1)).unsqueeze(1)

        q1, q2, _ = self.critic(state, act)
        critic_loss = F.mse_loss(q1, target_q) + F.mse_loss(q2, target_q) + F.mse_loss(v, target_v)
