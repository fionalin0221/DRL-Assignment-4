...
class SACAgent:
    def __init__(self, obs_dim, act_dim, act_limit, batch_size = 256, gamma=0.99, alpha=0.2, tau=0.005):
    ...
    def update(self):
        state, next_state, act, rew, done = self.sample()
        
        # Target action
        with torch.no_grad():
            next_act, next_logp = self.actor.sample(next_state)
            q1_target, q2_target = self.critic_target(next_state, next_act)
            q_target = torch.min(q1_target, q2_target) - self.alpha * next_logp # expectation of log π(a'|s') is entropy 
            critic_target = (rew + self.gamma * (1 - done) * q_target.squeeze(1)).unsqueeze(1)
       
        # Critic loss
        q1, q2 = self.critic(state, act)
        critic_loss = F.mse_loss(q1, critic_target) + F.mse_loss(q2, critic_target)
...
def train(env, epoches):
    ...
    for epoch in tqdm(range(epoches)):
        ...
        if actor_loss and critic_loss:
            tqdm.write(f"Episode {epoch} — Return: {ret:.2f}, Loss: {actor_loss:.4f}, {critic_loss:.4f}")
        else:
            tqdm.write(f"Episode {epoch} — Return: {ret:.2f}")
